...
function Namedle() {
  ...
  */
  ...
  const evaluateGuess = () => {
    ...
    const gameWon =
      updatedGuessStatus[currentGuess].filter(
        (item) => item === GuessTypes.CORRECT
      ).length === name.length;
    return gameWon;
  };
  const handleKeydown = useCallback(
    (code: string, key: string) => {
      ...
      if (
        (code === "Backspace" &&
          focusedInput === 0 &&
          inputGuesses[currentGuess][focusedInput] === "") ||
        (code !== "Enter" &&
          code !== "Backspace" &&
          focusedInput === name.length)
      ) {
        return;
      }
      ...
      if (code === "Backspace") {
        ...
        const message = `Game won at ${currentGuess + 1} attempt${
          currentGuess > 0 ? "s" : ""
        }!`;
        if (gameResult) {
          ...
          return (
            <input
              ...
              className={classNames(
                "caret-transparent text-md text-default-text border border-ocean-02 text-center size-tile-md focus:outline-none transition-all",
                {
                  "bg-correct": isCorrect,
                  "bg-present": isPresent,
                  "bg-absent": isAbsent,
                  "bg-default-background": isNone,
                }
              )}
  ...
  return (
    <div className="bg-ocean-04 h-full">
